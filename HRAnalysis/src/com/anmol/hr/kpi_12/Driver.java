...
public class Driver {
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
	    ...
	    {
	      System.err.println("HR Analysis KPI 12 Usage: HR Analysis <input-path> [...] <output-path>");
	      System.exit(2);
	    }
		Job exp = Job.getInstance(conf, "HR Analysis KPI 12");
		
		exp.setJarByClass(Driver.class);
		exp.setMapperClass(Mapper1.class);
		exp.setReducerClass(Reducer1.class);
		
		exp.setMapOutputKeyClass(Text.class);
		exp.setMapOutputValueClass(LongWritable.class);
		exp.setOutputKeyClass(Text.class);
		exp.setOutputValueClass(NullWritable.class);
